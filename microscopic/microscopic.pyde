...
class Amoeba:
    ...
    def __init__(self,x,y, xspeed, yspeed, diameter):
        ...
        self.nucleus = [
         self.diameter/200*int(random(-5,5)), # y-ccord
         self.diameter/random(1.5,2.5),       # x-ccord
        ...
        for i in range(4):
            self.wobbleradius.append(PVector(
                ...
                random(-xspeed*diameter/5,yspeed*diameter/5)
                ))
        self.t = 0.0
    ...
    def update(self):
        ...
        fill(self.nucleus[3])
        noStroke()
        ellipse(self.location.x + self.nucleus[0],self.location.y + self.nucleus[1],self.nucleus[2],self.nucleus[2])
